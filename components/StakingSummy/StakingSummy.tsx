    ...
    return (
        ...
        >
            <motion.div
                ...
                className="w-full sm:max-w-lg max-h-[90vh] rounded-t-3xl sm:rounded-3xl border border-white/8 relative flex flex-col overflow-hidden"
            ...
            >
                ...
                <div className="flex-1 overflow-y-auto p-5 space-y-4">
                            ...
                            <div className="grid grid-cols-3 gap-2">
                                {[
                                    { label: "APY", value: `${item.per_annum}%`, color: "text-emerald-400" },
                            ...
                            <table className="w-full">
                                <thead>
                                    <tr className="border-b border-white/8">
                                        <th className="text-left text-[12px] text-gray-600 uppercase tracking-wider font-bold px-5 py-4">Staking</th>
                                        <th className="text-left text-[12px] text-gray-600 uppercase tracking-wider font-bold px-5 py-4">Amount</th>
                                        <th className="text-left text-[12px] text-gray-600 uppercase tracking-wider font-bold px-5 py-4">Staking %</th>
                                        <th className="text-left text-[12px] text-gray-600 uppercase tracking-wider font-bold px-5 py-4">Method</th>
                                        <th className="text-left text-[12px] text-gray-600 uppercase tracking-wider font-bold px-5 py-4">Status</th>
                                        <th className="text-left text-[12px] text-gray-600 uppercase tracking-wider font-bold px-5 py-4">Unlock</th>
                                        <th className="text-center text-[12px] text-gray-600 uppercase tracking-wider font-bold px-5 py-4">Actions</th>
                                    </tr>
                                </thead>
                                <tbody>
                                            ...
                                            <td className="px-5 py-4">
                                                <div className="flex items-center justify-end gap-1.5 flex-wrap">
                                                    <button
                                                        ...
                                                        className="h-7 w-7 rounded-lg bg-white/5 border border-white/8 flex items-center justify-center text-gray-400 hover:text-emerald-400 hover:border-emerald-500/25 transition-all"
                                                        title="View"
                                                    >
                                                        <Eye size={12} />
                                                    </button>
                                                    {item.status === "RUNNING" && (
                                                        <>
                                                            <button
                                                                onClick={() => setAddRemoveItem(item)}
                                                                className="h-7 px-2 rounded-lg bg-white/5 border border-white/8 text-[10px] font-bold text-gray-400 hover:text-emerald-400 hover:border-emerald-500/25 transition-all"
                                                            >
                                                                +/−
                                                            </button>
                                                            {isPromo ? (
                                                                <button className="h-7 px-2 rounded-lg bg-amber-500/10 border border-amber-500/20 text-[10px] font-bold text-amber-400 hover:bg-amber-500/20 transition-all">
                                                                    Manage
                                                                </button>
                                                            ) : (
                                                                <button
                                                                    onClick={() => setUnlockItem(item)}
                                                                    className="h-7 px-2 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-[10px] font-bold text-emerald-400 hover:bg-emerald-500/20 transition-all"
                                                                >
                                                                    Unlock
                                                                </button>
                                                            )}
                            ...
                            {displayData.map((item, i) => {
                                ...
                                >
                                    ...
                                    <div className="grid grid-cols-3 gap-3">
                                        ...
                                        <div>
                                            <p className="text-[11px] text-gray-600 font-medium">APY</p>
...
const StakingSummary = () => {
    ...
    const [activePromo, setActivePromo] = useState<{ code: string; ytpAmount: number } | null>(null);
    const [confirming, setConfirming]   = useState(false);

    // Fetch YTP coin value + check for active mega offer promo
    useEffect(() => {
        (async () => {
            try {
                const [coinRes, taskRes] = await Promise.allSettled([
    ...
    const estReturn = numAmount > 0
        ? ((numAmount * plan.per_annum) / 100 * plan.validity / 365).toFixed(2)
        : "0.00";
    ...
    const now = new Date();
    const endDate = new Date(now.getTime() + plan.validity * 24 * 60 * 60 * 1000);
                            ...
                            >
                                ...
                                <AnimatePresence mode="wait">
                                    ...
                                    >
                                        ...
                                        <SummaryRow label="Refund Period"      value={`${plan.validity} Days`} />
                                        <SummaryRow label="Annual Yield (APY)" value={`${plan.per_annum}%`}    green />
                                        <SummaryRow
                                            label="Est. Return"
                                            value={`${estReturn} YTP`}
                                            sky
